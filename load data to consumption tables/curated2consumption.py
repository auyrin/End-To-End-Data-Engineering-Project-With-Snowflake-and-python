...
def create_promocode_dim(all_sales_df,session)-> None:
    ...
    #fetch existing product dim records.
    existing_promo_code_dim_df = session.sql("select promo_code as promotion_code, country, region from sales_dwh.consumption.promo_code_dim")
    ...
    promo_code_dim_df = promo_code_dim_df.selectExpr("sales_dwh.consumption.promo_code_dim_seq.nextval as promo_code_id_pk","promotion_code as promo_code", "country","region","isActive") 
...
def main():
    ...
    processed = session.sql("select order_code as order_id from sales_dwh.consumption.sales_fact")
    final_sales_df = final_sales_df.join(processed, processed['order_id']==final_sales_df['order_id'], join_type='leftanti')
    ...
    all_sales_df = all_sales_df.with_column( "promo_code", expr("case when promotion_code is null then 'NA' else promotion_code end"))
